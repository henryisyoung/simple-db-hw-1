...
public class BTreeFile implements DbFile {
	 ...
	 * 
	 * @param pid
	 *            - the id of the page to read from disk
			...
			if (id.pgcateg() == BTreePageId.ROOT_PTR) {
				...
				if (retval < BTreeRootPtrPage.getPageSize()) {
					throw new IllegalArgumentException(
							"Unable to read " + BTreeRootPtrPage.getPageSize() + " bytes from BTreeFile");
				}
				...
				if (bis.skip(BTreeRootPtrPage.getPageSize()
						+ (id.pageNumber() - 1) * BufferPool.getPageSize()) != BTreeRootPtrPage.getPageSize()
								+ (id.pageNumber() - 1) * BufferPool.getPageSize()) {
	 ...
	 *            - the transaction id
	 * @param dirtypages
	 *            - the list of dirty pages which should be updated with all new
	 *            dirty pages
	 * @param pid
	 *            - the current page being searched
	 * @param perm
	 *            - the permissions with which to lock the leaf page
	 * @param f
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid,
		...
		if (pid.pgcateg() == BTreePageId.LEAF) {
			...
			BTreeInternalPage internalPage = (BTreeInternalPage) this.getPage(tid, dirtypages, pid,
	 ...
	 * 
	 * @see #findLeafPage(TransactionId, HashMap, BTreePageId, Permissions,
	 *      Field)
	 * 
	 * @param tid
	 *            - the transaction id
	 ...
	 *            dirty pages
	 * @param page
	 *            - the leaf page to split
	 * @param field
	 *            - the key field of the tuple to be inserted after the split is
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page,
		...
		BTreeLeafPage NewRightPage = (BTreeLeafPage) this.getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
		BTreeLeafPage OldPage = page;
		BTreeLeafPage OldRightPage = null;
		Iterator<Tuple> reverseit = page.reverseIterator();
		if (OldPage.getRightSiblingId() != null) {
			OldRightPage = (BTreeLeafPage) this.getPage(tid, dirtypages, OldPage.getRightSiblingId(),
					Permissions.READ_WRITE);
			// Set Id for original right sibling
			OldRightPage.setLeftSiblingId(NewRightPage.getId());
			// Set Id for New Page right sibling
			NewRightPage.setRightSiblingId(OldPage.getRightSiblingId());
		}
		// Set Id for new page left sibling
		NewRightPage.setLeftSiblingId(page.getId());

		// Set Id for old page right sibling
		OldPage.setRightSiblingId(NewRightPage.getId());

		int halfposition = (int) Math.ceil((double) OldPage.getNumTuples() / 2);
		// Tuple temptuple=null;
		for (int i = 0; i < halfposition && reverseit.hasNext(); i++) {
			Tuple temptuple = reverseit.next();
			OldPage.deleteTuple(temptuple);
			NewRightPage.insertTuple(temptuple);
		}

		// update parent page
		Tuple toparent = NewRightPage.iterator().next();
		Field putparentfield = toparent.getField(keyField);
		BTreeInternalPage parentpage = this.getParentWithEmptySlots(tid, dirtypages, OldPage.getParentId(),
				putparentfield);
		OldPage.setParentId(parentpage.getId());
		NewRightPage.setParentId(parentpage.getId());
		// parent entry
		BTreeEntry newParentEntry = new BTreeEntry(putparentfield, OldPage.getId(), NewRightPage.getId());
		// newParentEntry.setRightChild(NewRightPage.getId());
		...
		if (field.compare(Op.GREATER_THAN_OR_EQ, toparent.getField(keyField))) {
			return NewRightPage;
		} else
			return OldPage;
		// some code goes here
		...
		// page and moving half of the tuples to the new page. Copy the middle
		// key up
		// into the parent page, and recursively split the parent as needed to
		// accommodate
		// the new entry. getParentWithEmtpySlots() will be useful here. Don't
		// forget to update
		// the sibling pointers of all the affected leaf pages. Return the page
		// into which a
	 ...
	 *            dirty pages
	 * @param page
	 *            - the internal page to split
	 * @param field
	 *            - the key field of the entry to be inserted after the split is
	 *            complete. Necessary to know which of the two pages to return.
	 * @see #getParentWithEmptySlots(TransactionId, HashMap, BTreePageId, Field)
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		BTreeInternalPage newInternalPage = (BTreeInternalPage) this.getEmptyPage(tid, dirtypages,
				BTreePageId.INTERNAL);

		Iterator<BTreeEntry> intpageit = page.reverseIterator();

		int halfposition = page.getNumEntries() / 2;
		BTreeEntry toparent = null;
		// Step 2:Split and modify child page
		for (int i = 0; i < halfposition; i++) {
			toparent = intpageit.next();
			page.deleteKeyAndRightChild(toparent);
			newInternalPage.insertEntry(toparent);
			// this.updateParentPointer(tid, dirtypages,
			// newInternalPage.getId(), toparent.getRightChild());
		}
		// Step 3:Modify parent page

		toparent = intpageit.next();
		Field midfield = toparent.getKey();
		BTreeInternalPage parent = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midfield);
		page.deleteKeyAndRightChild(toparent);
		toparent.setLeftChild(page.getId());
		toparent.setRightChild(newInternalPage.getId());
		parent.insertEntry(toparent);
		page.setParentId(parent.getId());
		newInternalPage.setParentId(parent.getId());
		// delete the page that has been passed to parent

		this.updateParentPointers(tid, dirtypages, page);
		this.updateParentPointers(tid, dirtypages, newInternalPage);
		...
		if (field.compare(Op.GREATER_THAN_OR_EQ, midfield)) {
			...
			return page;
		// Split the internal page by adding a new page on the right of the
		// existing
		// page and moving half of the entries to the new page. Push the middle
		// key up
		// into the parent page, and recursively split the parent as needed to
		// accommodate
		// the new entry. getParentWithEmtpySlots() will be useful here. Don't
		...
		// will be useful here. Return the page into which an entry with the
		// given key field
		// should be inserted.
	 ...
	 *            dirty pages
	 * @param parentId
	 *            - the id of the parent. May be an internal page or the RootPtr
	 *            page
	 * @param field
	 *            - the key of the entry which will be inserted. Needed in case
	 *            the parent must be split to accommodate the new entry
	...
	private BTreeInternalPage getParentWithEmptySlots(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		if (parentId.pgcateg() == BTreePageId.ROOT_PTR) {
			...
			BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) getPage(tid, dirtypages, BTreeRootPtrPage.getId(tableid),
					Permissions.READ_WRITE);
			BTreePageId prevRootId = rootPtr.getRootId(); // save prev id before
															// overwriting.
	 ...
	 * @param child
	 *            - id of the child node to be updated with the parent pointer
	 * @throws DbException
	 * @throws IOException
	 * @throws TransactionAbortedException
	 */
	private void updateParentPointer(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid,
			BTreePageId child) throws DbException, IOException, TransactionAbortedException {
	 ...
	 * @param page
	 *            - the parent page
	 * @see #updateParentPointer(TransactionId, HashMap, BTreePageId,
	 *      BTreePageId)
	 * 
	 * @throws DbException
	 * @throws IOException
	 ...
	 *         include many pages since parent pointers will need to be updated
	...
	public ArrayList<Page> insertTuple(TransactionId tid, Tuple t)
		...
		// get a read lock on the root pointer page and use it to locate the
		// root page
		BTreeRootPtrPage rootPtr = getRootPtrPage(tid, dirtypages);
		BTreePageId rootId = rootPtr.getRootId();

		if (rootId == null) { // the root has just been created, so set the root
	 ...
	 *            dirty pages
	 * @param page
	 *            - the page which is less than half full
	 * @see #handleMinOccupancyLeafPage(TransactionId, HashMap, BTreeLeafPage,
	 *      BTreeInternalPage, BTreeEntry, BTreeEntry)
	 * @see #handleMinOccupancyInternalPage(TransactionId, HashMap,
	...
	private void handleMinOccupancyPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePage page)
		...
		// find the left and right siblings through the parent so we make sure
		// they have
		// the same parent as the page. Find the entries in the parent
		// corresponding to
	 ...
	 *            dirty pages
	 * @param page
	 *            - the leaf page which is less than half full
	 * @param parent
	 *            - the parent of the leaf page
	 * @param leftEntry
	 *            - the entry in the parent pointing to the given page and its
	 *            left-sibling
	 * @param rightEntry
	 *            - the entry in the parent pointing to the given page and its
	 *            right-sibling
	 * @see #mergeLeafPages(TransactionId, HashMap, BTreeLeafPage,
	 *      BTreeLeafPage, BTreeInternalPage, BTreeEntry)
	...
	private void handleMinOccupancyLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page,
		...
		if (leftSiblingId != null) {
			...
			BTreeLeafPage rightSibling = (BTreeLeafPage) getPage(tid, dirtypages, rightSiblingId,
					Permissions.READ_WRITE);
			// if the right sibling is at minimum occupancy, merge with it.
			// Otherwise
			// steal some tuples from it
	 ...
	 *            - the leaf page which is less than half full
	 * @param sibling
	 *            - the sibling which has tuples to spare
	 * @param parent
	 *            - the parent of the two leaf pages
	 * @param entry
	 *            - the entry in the parent pointing to the two leaf pages
	 * @param isRightSibling
	 *            - whether the sibling is a right-sibling
	 * 
	 * @throws DbException
	 */
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling, BTreeInternalPage parent,
			BTreeEntry entry, boolean isRightSibling) throws DbException {

		Iterator<Tuple> copytuple = null;
		Tuple temptuple = null;
		int midnumber = (page.getNumTuples() + sibling.getNumTuples()) / 2;
		if (isRightSibling) {
			copytuple = sibling.iterator();
		} else {
			copytuple = sibling.reverseIterator();
		}

		while (page.getNumTuples() < midnumber) {
			temptuple = copytuple.next();
			sibling.deleteTuple(temptuple);
			page.insertTuple(temptuple);
		}
		if (isRightSibling) {
			entry.setKey(copytuple.next().getField(this.keyField));
		} else {
	 ...
	 *            dirty pages
	 * @param page
	 *            - the internal page which is less than half full
	 * @param parent
	 *            - the parent of the internal page
	 * @param leftEntry
	 *            - the entry in the parent pointing to the given page and its
	 *            left-sibling
	 * @param rightEntry
	 *            - the entry in the parent pointing to the given page and its
	 *            right-sibling
	 * @see #mergeInternalPages(TransactionId, HashMap, BTreeInternalPage,
	 *      BTreeInternalPage, BTreeInternalPage, BTreeEntry)
	 * @see #stealFromLeftInternalPage(TransactionId, HashMap,
	 *      BTreeInternalPage, BTreeInternalPage, BTreeInternalPage, BTreeEntry)
	 * @see #stealFromRightInternalPage(TransactionId, HashMap,
	 *      BTreeInternalPage, BTreeInternalPage, BTreeInternalPage, BTreeEntry)
	 * 
	 * @throws DbException
	 * @throws IOException
	 ...
	 *            - the internal page which is less than half full
	 * @param leftSibling
	 *            - the left sibling which has entries to spare
	 * @param parent
	 *            - the parent of the two internal pages
	 * @param parentEntry
	 *            - the entry in the parent pointing to the two internal pages
	 * @see #updateParentPointers(TransactionId, HashMap, BTreeInternalPage)
	 * 
	 * @throws DbException
	 * @throws IOException
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int midnum = (page.getNumEntries() + leftSibling.getNumEntries()) / 2;
		Iterator<BTreeEntry> reverseIterator = leftSibling.reverseIterator();
		BTreeEntry tempEntry;
		while (page.getNumEntries() < midnum) {
			tempEntry = reverseIterator.next();
			Field midkey = tempEntry.getKey();
			leftSibling.deleteKeyAndRightChild(tempEntry);
			// modify key
			tempEntry.setKey(parentEntry.getKey());
			parentEntry.setKey(midkey);

			parent.updateEntry(parentEntry);
			tempEntry.setLeftChild(tempEntry.getRightChild());
			tempEntry.setRightChild(page.iterator().next().getLeftChild());

			page.insertEntry(tempEntry);
			// leftSibling.deleteKeyAndRightChild(tempEntry);
		}
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int midnum = (page.getNumEntries() + rightSibling.getNumEntries()) / 2;
		Iterator<BTreeEntry> iterator = rightSibling.iterator();
		BTreeEntry tempEntry = null;
		while (page.getNumEntries() < midnum) {
			tempEntry = iterator.next();
			rightSibling.deleteKeyAndLeftChild(tempEntry);
			Field originalField = tempEntry.getKey();
			tempEntry.setKey(parentEntry.getKey());
			parentEntry.setKey(originalField);

			parent.updateEntry(parentEntry);
			
			tempEntry.setRightChild(tempEntry.getLeftChild());
			tempEntry.setLeftChild(page.reverseIterator().next().getRightChild());

			// tempEntry.setKey(originalField);
			page.insertEntry(tempEntry);
			// leftSibling.deleteKeyAndRightChild(tempEntry);
		}
		// parentEntry.setKey(tempEntry.getKey());
		//
		// parent.updateEntry(parentEntry);
	 ...
	 *            - the right leaf page
	 * @param parent
	 *            - the parent of the two pages
	 * @param parentEntry
	 *            - the entry in the parent corresponding to the leftPage and
	 *            rightPage
	 * @see #deleteParentEntry(TransactionId, HashMap, BTreePage,
	 ...
	 * @throws TransactionAbortedException
	 */
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage leftPage,
		...
		// connect left page and original right page
		leftPage.setRightSiblingId(rightPage.getRightSiblingId());
		if (rightPage.getRightSiblingId() != null) {
			BTreeLeafPage oldRightSibling = (BTreeLeafPage) this.getPage(tid, dirtypages, rightPage.getRightSiblingId(),
					Permissions.READ_WRITE);

			oldRightSibling.setLeftSiblingId(leftPage.getId());
		}
		...
		// Delete the entry in the parent corresponding to the two pages that
		// are merging -
	 ...
	 * @throws TransactionAbortedException
	 */
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeInternalPage leftPage,
		...
		while (iterator.hasNext()) {
			BTreeEntry tempEntry = iterator.next();
	 ...
	 *            dirty pages
	 * @param leftPage
	 *            - the child remaining after the key and right child are
	 *            deleted
	 * @param parent
	 *            - the parent containing the entry to be deleted
	 * @param parentEntry
	 ...
	 * @throws TransactionAbortedException
	 */
	private void deleteParentEntry(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePage leftPage,
			BTreeInternalPage parent, BTreeEntry parentEntry)
			throws DbException, IOException, TransactionAbortedException {

		// delete the entry in the parent. If
		// the parent is below minimum occupancy, get some tuples from its
		// siblings
		...
		if (parent.getNumEmptySlots() == parent.getMaxEntries()) {
			...
			// In this case, the parent (root node) should be deleted, and the
			// merged
	...
	public ArrayList<Page> deleteTuple(TransactionId tid, Tuple t)
		...
		BTreePageId pageId = new BTreePageId(tableid, t.getRecordId().getPageId().pageNumber(), BTreePageId.LEAF);
		BTreeLeafPage page = (BTreeLeafPage) getPage(tid, dirtypages, pageId, Permissions.READ_WRITE);
		page.deleteTuple(t);

		// if the page is below minimum occupancy, get some tuples from its
		// siblings
		// or merge with one of the siblings
		int maxEmptySlots = page.getMaxTuples() - page.getMaxTuples() / 2; // ceiling
	 ...
	 *            dirty pages
	 * @param pgcateg
	 *            - the BTreePageId category of the new page. Either LEAF,
	 *            INTERNAL, or HEADER
	 ...
	 *            dirty pages
	 * @param emptyPageNo
	 *            - the page number of the empty page
	...
	protected void setEmptyPage(TransactionId tid, HashMap<PageId, Page> dirtypages, int emptyPageNo)
		...
		// truncate the file
		// @TODO: Commented out because we should probably do this somewhere
		// else in case the transaction aborts....
		// synchronized(this) {
		...
		// otherwise, get a read lock on the root pointer page and use it to
		// locate
		...
		// iterate through all the existing header pages to find the one
		// containing the slot
		...
		// Add header pages until we have one with a slot corresponding to
		// emptyPageNo
	 ...
	 *            - the transaction id
	 * @param ipred
	 *            - the index predicate value to filter on
...
class BTreeFileIterator extends AbstractDbFileIterator {
	 ...
	 * 
	 * @param f
	 *            - the BTreeFile containing the tuples
	 * @param tid
	 *            - the transaction id
	 */
		...
		while (it != null) {
			...
			while (it.hasNext()) {
				...
				if (t.getField(f.keyField()).compare(ipred.getOp(), ipred.getField())) {
					...
					// if the predicate was not satisfied and the operation is
					// less than, we have
					// hit the end
					return null;
				} else if (ipred.getOp() == Op.EQUALS
						&& t.getField(f.keyField()).compare(Op.GREATER_THAN, ipred.getField())) {
					// if the tuple is now greater than the field passed in and
					// the operation
