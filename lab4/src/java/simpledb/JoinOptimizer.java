...
public class JoinOptimizer {
	...
	public static DbIterator instantiateJoin(LogicalJoinNode lj, DbIterator plan1, DbIterator plan2)
		...
		if (lj.p == Predicate.Op.EQUALS) {
			...
			try {
				...
				j = (DbIterator) ct.newInstance(new Object[] { p, plan1, plan2 });
			} catch (Exception e) {
				j = new Join(p, plan1, plan2);
			}
		} else {
	...
	public Vector<LogicalJoinNode> orderJoins(HashMap<String, TableStats> stats,
		...
		Vector<LogicalJoinNode> res;
		for (int i = 0; i <=numjoins; i++) {
			Set<Set<LogicalJoinNode>> subset = enumerateSubsets(joins, i);
			for (Set<LogicalJoinNode> jn : subset) {
				CostCard bestcostplan = null;
				double bestcostnow = Double.MAX_VALUE;

				for (LogicalJoinNode l : jn) {
					CostCard subcostplan = computeCostAndCardOfSubplan(stats, filterSelectivities, l, jn, bestcostnow,
							pc);
					if (subcostplan != null && subcostplan.cost < bestcostnow) {
						bestcostnow = subcostplan.cost;
						bestcostplan = subcostplan;
					}
				}
				if (bestcostplan != null) {
					pc.addPlan(jn, bestcostnow, bestcostplan.card, bestcostplan.plan);
				}
			}
		}
		res = pc.getOrder(new HashSet<LogicalJoinNode>(joins));
		if (explain) {
		...
		for (LogicalJoinNode j : js) {
			...
			if (n == null) { // never seen this table before
				n = new DefaultMutableTreeNode(
						j.t1Alias + " (Cost = " + stats.get(table1Name).estimateScanCost() + ", card = "
